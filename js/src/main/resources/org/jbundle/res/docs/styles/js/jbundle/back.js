...
(function(){ 
	...
	function setHash(h){
		if(!h){ h = ""; }
		var encHash = encodeURIComponent(h);
		currentHash = encHash;	// Don't trigger a hash change on setHash (since this is called from push history)
		window.location.hash = encHash;
	...
	var currentHash = (typeof(window) !== "undefined") ? getHash() : "";
	var onHashChangeFunction = null;
	...
	function checkLocation(){
		if(!changingUrl){
			...
			if (hash != currentHash)
				if (encodeURIComponent(hash) != currentHash)
			{
				currentHash = hash;
	...
	function handleHashChange(hash){
		if (onHashChangeFunction)
			onHashChangeFunction(hash);
	};
	
	back.init = function(hashChangeFunction){
		...
		currentHash = getHash();
		onHashChangeFunction = hashChangeFunction;
		if(!locationTimer){
